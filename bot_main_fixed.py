...
# ───────────────────────────  Константы и пути  ────────────────────────────
DATA_DIR = pathlib.pathlib.Path("data")
...
async def export_analysis_json(analysis: Dict, chat_id: int, bot: Bot) -> bool:
    ...
    try:
        ...
        temp_dir = pathlib.pathlib.Path(tempfile.mkdtemp())
...
async def on_analyze_only(call: CallbackQuery):
    ...
    try:
        # Выполняем анализ
        file_path = pathlib.pathlib.Path(session['temp_file'])
...
async def finish_processing(
    ...
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = pathlib.pathlib.Path(tempfile.mkdtemp()) / f"vk_data_results_{ts}.xlsx"
